...
def preproc(text: str) -> str:
    return unicodedata.normalize(
        "NFKC", mpn.normalize(text).translate(non_printable_map)
    )
...
def apply_variations(xx: pd.Series, yy: pd.Series) -> tuple[pd.Series, pd.Series]:
    ...
    for i in range(N):
        ...
        if matches_x and matches_x == matches_y:
            ...
            def replace_seq(original, rand_names_seq):
                out = []
                last_idx = 0
                for m, newname in zip(
                    pattern_names_re.finditer(original), rand_names_seq
                ):
    ...
    delete_idxs = idxs[
        n_upper + n_nocap + n_emoji : n_upper + n_nocap + n_emoji + n_delete
    ]
    ...
    n_newline = N // 8
    newline_idxs = idxs[
        n_upper + n_nocap + n_emoji + n_delete : n_upper
        + n_nocap
        + n_emoji
        + n_delete
        + n_newline
