...
def download_and_unpack_bz2(link: str, output_file: str):
    ...
    with (
        bz2.BZ2File(compressed_file, "rb") as file_in,
        open(output_file, "wb") as uncompressed_out,
    ):
        shutil.copyfileobj(file_in, uncompressed_out)
    print(f"Unpacked to {output_file}.")


def download_and_unpack_links_file(
    tatoeba_path: str, redownload: bool = False, verbose: bool = False
):
...
def download_and_unpack_tatoeba(
    tatoeba_path: str, source_lang: str, redownload: bool = False, verbose: bool = False
):
...
def main_download(
    ...
    tatoeba_path: str = "data/tatoeba",
    verbose: bool = False,
):
    print("Downloading necessary Tatoeba files...")
    download_and_unpack_links_file(
        tatoeba_path=tatoeba_path, redownload=redownload, verbose=verbose
    )
    for src_lang in tqdm(source_langs):
        download_and_unpack_tatoeba(
